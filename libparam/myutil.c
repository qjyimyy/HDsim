...
void unparse_tlt(struct lp_tlt *tlt, FILE *outfile, char *infile) {
    ...
    switch (tlt->what) {
        case TLT_BLOCK:
            unparse_block(tlt->it.block, outfile);
            break;
        case TLT_TOPO:
            unparse_tl_topospec(tlt->it.topo, outfile);
            break;
        case TLT_INST:
            unparse_inst(tlt->it.inst, outfile);
            break;
        default:
            ddbg_assert(0);
            break;
...
int check_types(struct lp_block *b) {
    ...
    for (c = 0; c < b->params_len; c++) {
        if (b->params[c]) {
            ...
            else if (lp_modules[b->type]->modvars[lp_param_name(b->type, b->params[c]->name)].type != PTYPE(b->params[c])) {
                /* implicitly convert ints to doubles */
                if ((lp_modules[b->type]->modvars[lp_param_name(b->type, b->params[c]->name)].type == D) && (PTYPE(b->params[c]) == I)) {
                    b->params[c]->v->t = D;
...
int lp_param_name(int m, char *n) {
    ...
    while ((c < lp_modules[m]->modvars_len) && strcmp(lp_modules[m]->modvars[c].name, n)) c++;
...
int lp_loadparams(void *it, struct lp_block *b, struct lp_mod *m) {
    int c;
    int needed = 0;
