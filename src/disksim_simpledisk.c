...
struct simpledisk *getsimpledisk(int devno) {
    ...
    if (disksim->simplediskinfo->simpledisks[devno] == NULL) {
        return disksim->simplediskinfo->simpledisks[0];
    }
    ASSERT1((devno >= 0) && (devno < MAXDEVICES), "devno", devno);
...
double simpledisk_get_servtime(int devno, ioreq_event *req, int checkcache, double maxtime) {
    simpledisk_t *currdisk = getsimpledisk(devno);
    // 轨道的号
    currdisk->tape_trackno = req->blkno / currdisk->tape_block_of_tracks;

    int curr = req->blkno - (currdisk->tape_trackno) * currdisk->tape_block_of_tracks;

    currdisk->tape_track = currdisk->tape_trackno % 2 == 0 ? curr : currdisk->tape_block_of_tracks - curr;

    double time = 0;

    // 倒带的时间
    double time1 = abs(currdisk->stat.tape_track - currdisk->tape_track) * currdisk->spoolrate;

    // 磁头移动的时间
    double time2 = abs(currdisk->stat.tape_trackno - currdisk->tape_trackno) * currdisk->headtime;
    double time3 = 0;

    int flag = (int)maxtime;
    if (flag) {
        time3 = abs(req->bcount) * currdisk->readrate;
    } else {
        time3 = abs(req->bcount) * currdisk->writerate;
    }

    int rel = req->blkno + req->bcount;

    currdisk->stat.tape_trackno = rel / currdisk->tape_block_of_tracks;

    curr = rel - (currdisk->stat.tape_trackno) * currdisk->tape_block_of_tracks;
    currdisk->stat.tape_track = currdisk->stat.tape_trackno % 2 == 0 ? curr : currdisk->tape_block_of_tracks - curr;

    time = max(time1, time2) + time3;

    currdisk->count++;
    currdisk->xfertimeCount += time3;
    currdisk->headatimeCount += time2;
    currdisk->spooltimeCount += time1;
    return time;
}
...
void simpledisk_printstats(void) {
    ...
    // for (i = 0; i < MAXDEVICES; i++) {
    //     simpledisk_t *currdisk = getsimpledisk(i);
    //     fprintf(outputfile, "tape  count  %d:\n\n", currdisk->count);
    //     fprintf(outputfile, "tape headtime  %lf ms:\n\n", (double)(currdisk->headatimeCount / currdisk->count));
    //     fprintf(outputfile, "tape  xfertime  %lf ms:\n\n", (double)(currdisk->xfertimeCount / currdisk->count));
    //     fprintf(outputfile, "tape  spooltime  %lf ms :\n\n", (double)(currdisk->spooltimeCount / currdisk->count));
    ...
    for (i = 0; i < numsimpledisks; i++) {
        simpledisk_t *currdisk = getsimpledisk(set[i]);
        fprintf(outputfile, "tape  count  %d:\n\n", currdisk->count);
        fprintf(outputfile, "tape headatime  %lf ms:\n\n", (double)(currdisk->headatimeCount / currdisk->count));
        fprintf(outputfile, "tape  xfertime  %lf ms:\n\n", (double)(currdisk->xfertimeCount / currdisk->count));
        fprintf(outputfile, "tape  spooltime  %lf ms :\n\n", (double)(currdisk->spooltimeCount / currdisk->count));
