  ...
  disk *currdisk;
   ...
   if (!currdisk) {
      //fprintf (stderr, "getdisk returning a non-initialized currdisk\n");
           currdisk = disksim->diskinfo->disks[0];
            }
             return (currdisk);
...
static int disk_get_number_of_blocks(int n) {
    return disksim->diskinfo->disks[0]->model->dm_sectors;
}

static int disk_get_numcyls(int n) {
    return disksim->diskinfo->disks[0]->model->dm_cyls;
}
...
{
    struct dm_pbn pbn;
    struct dm_disk_if *d = disksim->diskinfo->disks[0]->model;
