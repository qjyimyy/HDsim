    ...
    */
    ...
    while (depth) {
        ...
        switch (currbus->slots[(slotno.byte[depth] >> 4)].devtype) {
            case CONTROLLER:
                ret = controller_get_data_transfered(checkdevno, curr->devno);
                break;

            default:
                fprintf(stderr, "Invalid device type in bus_get_data_transfered\n");
                exit(1);
        }
        if (ret != -1) {
            return (ret);
...
bus *bus_copy(bus *orig) {
    ...
    if (result)
        return (bus *)memcpy(result, orig, sizeof(bus));
    else
        return (bus *)0;
}
        ...
        if (type == lp_mod_name("disksim_ctlr")) {
            ...
            if (!load_ctlr_topo(ts, &busno)) {
                ...
                return 0;
                   ...
                   (type == lp_mod_name("memsmodel_mems")) || /*SSD:*/
