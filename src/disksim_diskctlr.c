...
static void disk_buffer_sector_done(disk *currdisk, ioreq_event *curr);

static void disk_buffer_seekdone(disk *currdisk, ioreq_event *curr);
...
static void disk_release_hda(disk *currdisk,
                             diskreq *currdiskreq);
...
static ioreq_event *disk_buffer_transfer_size(disk *currdisk,
                                              diskreq *currdiskreq,
                                              ioreq_event *curr);

static void disk_activate_read(disk *currdisk,
                               diskreq *currdiskreq,
                               int setseg,
                               int ok_to_check_bus);

static void disk_activate_write(disk *currdisk,
                                diskreq *currdiskreq,
                                int setseg,
                                int ok_to_check_bus);

static double disk_buffer_estimate_acctime(disk *currdisk,
                                           ioreq_event *curr,
                                           double maxtime);
...
static void disk_send_event_up_path(ioreq_event *curr, double delay) {
    ...
    if (currdisk->busowned == -1) {
        ...
        if (curr->next) {
            fprintf(stderr,
                    "Multiple bus requestors detected in "
                    "disk_send_event_up_path\n");
...
static void disk_check_prefetch_swap(disk *currdisk) {
    ...
    {
        int ispureprefetch =
            (currdisk->effectivehda->seg->access->flags & BUFFER_BACKGROUND) && (currdisk->effectivehda->seg->access->flags & READ);

        ddbg_assert2(ispureprefetch,
                     "effectivehda->seg->access is not a pure "
                     "prefetch");
    }
...
static void disk_find_new_seg_owner(disk *currdisk, segment *seg) {
    ...
    } else if (currdisk->effectivehda && (currdisk->effectivehda->seg == seg)) {
...
static void disk_release_hda(disk *currdisk, diskreq *currdiskreq) {
    ...
    if (!currdiskreq->ioreqlist) {
        ...
        if ((currdisk->preseeking == PRESEEK_BEFORE_COMPLETION) || ((currdisk->preseeking == PRESEEK_DURING_COMPLETION) && (currdiskreq->flags & COMPLETION_SENT))) {
            int cleanOrEmpty;
            release_hda = TRUE;
            ...
            seg->state = (disk_buffer_state_t)(currdisk->enablecache || seg->diskreqlist->seg_next || cleanOrEmpty);
        }
    } else if ((currdiskreq->flags & COMPLETION_RECEIVED) || (currdisk->preseeking == PRESEEK_BEFORE_COMPLETION) || ((currdisk->preseeking == PRESEEK_DURING_COMPLETION) && (currdiskreq->flags & COMPLETION_SENT))) {
        release_hda = TRUE;
        ...
        if (!tmpdiskreq) {
            ...
            //	    fflush(outputfile);
            //	}
            ddbg_assert3(((currdisk->numdirty >= 0) && (currdisk->numdirty <= currdisk->numwritesegs)),
...
disk_request_needs_bus(disk *currdisk,
    ...
    if (seg && (currdiskreq != seg->recyclereq) && tmpioreq && !(currdiskreq->flags & COMPLETION_SENT)) {
        ...
        if (currdiskreq->ioreqlist->flags & READ) {
            ...
            if ((seg->endblkno > currdiskreq->outblkno) && (seg->startblkno <= currdiskreq->outblkno) && (!check_watermark || (seg->endblkno >= (tmpioreq->blkno + tmpioreq->bcount)) || ((seg->endblkno - currdiskreq->outblkno) >= currdiskreq->watermark))) {
                if (currdisk->outwait) {
                    busioreq = currdisk->outwait;
                    currdisk->outwait = NULL;
                    busioreq->time = simtime;
            ...
            if ((currdiskreq->inblkno >= (tmpioreq->blkno + tmpioreq->bcount)) || ((currdiskreq->outblkno == currdiskreq->ioreqlist->blkno) && (currdiskreq->hittype != BUFFER_APPEND)) || ((seg->endblkno == currdiskreq->outblkno) && (seg->endblkno < (tmpioreq->blkno + tmpioreq->bcount)) && ((seg->endblkno - seg_owner->inblkno) < seg->size) && (!check_watermark || ((seg->endblkno - currdiskreq->inblkno) <= currdiskreq->watermark)))) {
                ...
                if ((currdiskreq->outblkno == currdiskreq->ioreqlist->blkno) && (currdiskreq->hittype != BUFFER_APPEND)) {
...
disk_select_bus_request(disk *currdisk, ioreq_event **busioreq) {
    ...
    while (currdiskreq) {
        ...
        if (currdiskreq->seg && (currdiskreq->seg->recyclereq == currdiskreq)) {
            // dont do anything
        ...
        } else if ((best_value <= 7) && (currdisk->fastwrites == LIMITED_FASTWRITE) && (currdiskreq->hittype == BUFFER_APPEND)) {
            currioreq = disk_request_needs_bus(currdisk, currdiskreq, TRUE);
            if (currioreq) {
                curr_value = 7;
            }
        } else if ((best_value <= 6) && (currdisk->sneakyintermediatereadhits) && (currdiskreq->ioreqlist->flags & READ)) {
            if (currdiskreq->seg) {
                currioreq = disk_request_needs_bus(currdisk, currdiskreq, TRUE);
                if (currioreq) {
                    tmpioreq = currdiskreq->ioreqlist;
        ...
        } else if ((best_value <= 2) && (currdisk->writeprebuffering) && !(currdiskreq->ioreqlist->flags & READ)) {
            if (currdiskreq->seg) {
                currioreq = disk_request_needs_bus(currdisk, currdiskreq, TRUE);
                if (currioreq) {
                    tmpioreq = currdiskreq->ioreqlist;
...
disk_check_bus(disk *currdisk, diskreq *currdiskreq) {
    ...
    if (nextdiskreq) {
        ...
        if ((nextdiskreq != currdisk->currentbus) && (nextdiskreq != currdisk->effectivebus)) {
...
disk_initiate_seek(
    ...
    if ((curr->flags & BUFFER_BACKGROUND) && (curr->flags & READ)) {
        if ((currdisk->contread == BUFFER_READ_UNTIL_CYL_END) && (seekdist != 0)) {
            return FALSE;
        }

        if ((currdisk->contread == BUFFER_READ_UNTIL_TRACK_END) && ((seekdist != 0) |
...
disk_check_hda_pending(disk *currdisk,
    ...
    // ... or is the request a WRITE?
        ...
        if ((nextdiskreq->hittype != BUFFER_APPEND) || (seg->access->flags & READ) || (seg->access->blkno != nextdiskreq->ioreqlist->blkno) || (seg->access->bcount != 1) || (seg->access->time != simtime)) {
    ...
    if (ok_to_check_bus && (currdisk->extradisc_diskreq != nextdiskreq)) {
...
static void disk_check_hda(disk *currdisk,
    ...
    if (!immediate_release && (seg->state == BUFFER_READING) && (seg->access->flags & BUFFER_BACKGROUND)) {
        disk_check_prefetch_swap(currdisk);
    }
...
disk_buffer_attempt_access_swap(disk *currdisk,
    ...
    if ((effective == currdisk->currenthda) && (seg->access->flags & BUFFER_BACKGROUND) && (!SWAP_FORWARD_ONLY || !effective->ioreqlist || (effective->ioreqlist->blkno < currdiskreq->ioreqlist->blkno))) {
        if (effective->flags & COMPLETION_RECEIVED) {
            ...
            if ((seg->endblkno >= (currioreq->blkno + currioreq->bcount)) && (seg->endblkno < seg->maxreadaheadblkno)) {
                seg->access->flags |= BUFFER_BACKGROUND;
            }
        }
        currdisk->effectivehda = currdisk->currenthda = NULL;
...
disk_activate_read(disk *currdisk,
    ...
    if (!(currdiskreq->flags & HDA_OWNED) && currdisk->effectivehda && (currdisk->effectivehda != currdiskreq) && (currdiskreq->hittype != BUFFER_COLLISION) && (!currdiskreq->seg || (currdiskreq->seg->recyclereq == currdiskreq) || (currdiskreq->seg->state != BUFFER_DIRTY)) && disk_buffer_stopable_access(currdisk, currdiskreq)) {
        if ((currdiskreq->seg != currdisk->effectivehda->seg) || (currdiskreq->hittype == BUFFER_NOMATCH)) {
    ...
    if (!(currdiskreq->flags & HDA_OWNED) && (currdiskreq->hittype != BUFFER_COLLISION)) {
        disk_check_hda(currdisk, currdiskreq, ok_to_check_bus);
    }

    if (!(currdiskreq->flags & HDA_OWNED) && (currdisk->currentbus == currdiskreq)) {
        currioreq = currdiskreq->ioreqlist;
        while (currioreq) {
...
disk_activate_write(disk *currdisk,
    ...
    // DISK device path
    if (!(currdiskreq->flags & HDA_OWNED) && currdisk->effectivehda && (currdisk->effectivehda != currdiskreq) && disk_buffer_stopable_access(currdisk, currdiskreq)) {
    ...
    switch (new_diskreq->hittype) {
        ...
        case BUFFER_WHOLE:
           ...
           ELSE
            ...
            if (currdisk->neverdisconnect || (enough_ready_to_go && !currdisk->currentbus && !currdisk->const_acctime && ((currdisk->sneakyfullreadhits && (new_diskreq->hittype == BUFFER_WHOLE)) || (currdisk->sneakypartialreadhits && (new_diskreq->hittype == BUFFER_PARTIAL)) || (!currdisk->pendxfer && (!currdisk->effectivehda || disk_buffer_stopable_access(currdisk, new_diskreq)))))) {
...
static void disk_write_arrive(disk *currdisk,
    ...
    if (currdisk->neverdisconnect || (seg && !currdisk->currentbus && !currdisk->const_acctime && (currdisk->writeprebuffering || (!currdisk->pendxfer && (!currdisk->effectivehda || disk_buffer_stopable_access(currdisk, new_diskreq)))))) {
        // path if effectivehda is NULL
        if ((new_diskreq->flags & EXTRA_WRITE_DISCONNECT) && !currdisk->const_acctime) {
...
static void disk_disconnect(ioreq_event *curr) {
    ...
    if (currdiskreq == currdisk->currentbus) {
        if ((currdiskreq->flags & EXTRA_WRITE_DISCONNECT) && (currdisk->extradisc_diskreq == currdiskreq)) {
...
static void disk_completion(ioreq_event *curr) {
    ...
    // printf("tagID = %d, tagIDBegin = %d, tagIDEnd = %d\n", curr->tagID, disksim->tagIDBegin, disksim->tagIDEnd);
...
disk_buffer_request_complete(disk *currdisk, diskreq *currdiskreq) {
    ...
    if (background && reading && (seg->endblkno == seg->minreadaheadblkno) && ((currdisk->preseeking == PRESEEK_BEFORE_COMPLETION) || (currdiskreq->flags & COMPLETION_RECEIVED) || ((currdisk->preseeking == PRESEEK_DURING_COMPLETION) && (currdiskreq->flags & COMPLETION_SENT)))) {
        hold_type = seg->access->type;
        seg->access->type = NULL_EVENT;
        disk_check_prefetch_swap(currdisk);
        if ((seg->outstate == BUFFER_PREEMPT) || (currdisk->effectivehda != currdiskreq)) {
            return (TRUE);
        }
        ...
        ioqueue_print_contents(currdisk->queue);
#endif
        if (!reading || (currdisk->contread == BUFFER_NO_READ_AHEAD) || (seg->endblkno >= seg->maxreadaheadblkno) || (currdisk->queue->base.listlen > 1)) {
            seg->access->type = NULL_EVENT;
        } else {
            seg->access->flags |= BUFFER_BACKGROUND;
            reqdone = FALSE;
        ...
        currdisk->stat.latency = (double)0.0;
        if (!reqdone && ((currdisk->preseeking == PRESEEK_BEFORE_COMPLETION) || (currdiskreq->flags & COMPLETION_RECEIVED) || ((currdisk->preseeking == PRESEEK_DURING_COMPLETION) && (currdiskreq->flags & COMPLETION_SENT)))) {
            hold_type = seg->access->type;
            seg->access->type = NULL_EVENT;
            disk_check_prefetch_swap(currdisk);
            if ((seg->outstate == BUFFER_PREEMPT) || (currdisk->effectivehda != currdiskreq)) {
    ...
    if (reqdone) {
        if (!reading) {
            ...
            if (!(currdiskreq->flags & COMPLETION_SENT) && (seg->outstate == BUFFER_IDLE)) {
                disk_check_bus(currdisk, currdiskreq);
            }
        }
        seg->access->type = NULL_EVENT;
...
disk_buffer_transfer_size(disk *currdisk,
    ...
    if (tmpioreq->flags & READ) {
        ...
        if ((currdiskreq->outblkno == (tmpioreq->blkno + tmpioreq->bcount)) && (currdiskreq->inblkno >= currdiskreq->outblkno)) {
            if (tmpioreq->next) {
      ...
      fprintf (outputfile, "%f Start transfer of %d sectors of %d (from %d)\n", simtime, seg->outbcount, seg->reqlist->bcount, seg->reqlist->blkno);
         */
        if ((!(currdiskreq->flags & SEG_OWNED) && (seg->endblkno < currdiskreq->outblkno)) || (seg->outbcount <= 0)) {
            if (currdisk->hold_bus_for_whole_write_xfer || currdisk->neverdisconnect) {
                if ((disk_printhack > 1) && (simtime >= disk_printhacktime)) {
    ...
    if (currioreq->flags & READ) {
      ...
      return(FALSE);
        ...
        if (currdiskreq->seg) {
            ...
            disk_buffer_select_segment(currdisk, currdiskreq, FALSE);
            if (currdiskreq->seg) {
                if ((currdiskreq->seg->state == BUFFER_WRITING) || (currdiskreq->seg->state == BUFFER_DIRTY)) {
                    if (!disk_buffer_reusable_segment_check(currdisk, currdiskreq->seg)) {
                        currdiskreq->hittype = BUFFER_NOMATCH;
                        currdiskreq->seg = NULL;
                        return (FALSE);
                ...
                if ((currdiskreq->hittype == BUFFER_COLLISION) || !disk_buffer_recyclable_segment(currdisk, TRUE)) {
                    return (FALSE);
                }
            }
        }
...
int disk_buffer_stopable_access(disk *currdisk, diskreq *currdiskreq) {
    ...
    if (seg->state == BUFFER_READING) {
       ...
       rather than take the hit immediately.)
        ...
        if ((!(effective->flags & COMPLETION_SENT) && (currdisk->preseeking != PRESEEK_BEFORE_COMPLETION)) || (!(effective->flags & COMPLETION_RECEIVED) && (currdisk->preseeking == NO_PRESEEK))) {
            return (FALSE);
        }
        ...
        if ((!currdisk->disconnectinseek) && ((seg->access->type == DEVICE_BUFFER_SEEKDONE) || (!currdisk->stopinsector))) {
            int rv = removefromintq((event *)seg->access);
            ddbg_assert3(rv != 0,
        ...
        if ((seg->access->type == DEVICE_BUFFER_SEEKDONE) || (!currdisk->stopinsector)) {
...
disk_buffer_seekdone(disk *currdisk, ioreq_event *curr) {
...

    {
        ...
        rv = currdisk->model->layout->dm_get_track_boundaries(currdisk->model,
                                                              &pbn,
                                                              &first,
                                                              &last,
                                                              &remapsector);
    ...
    if ((currdisk->readanyfreeblocks) && (curr->flags & READ) && (curr->blkno != first)) {
        curr->blkno = first;
        immed = TRUE;
    }
...
disk_buffer_sector_done(disk *currdisk, ioreq_event *curr) {
    ...
    if (curr->bcount == 1) {
        if (currblkno == firstblkno) {
             ...
             */
            ddbg_assert3(((seg->state == BUFFER_READING) && currdisk->readanyfreeblocks),
        ...
        } else if ((currdisk->immed) && (currdisk->immedstart >= next)) {
    ...
    // is it firstblkno > last ok?
        ...
        if ((currdiskreq != seg->recyclereq) && (currcause == -2)  // DM_REMAPPED
            && (seg->state == BUFFER_READING) && (currdiskreq->ioreqlist) && (currdisk->readanyfreeblocks) && (currblkno >= 0) && (currblkno == (seg->endblkno - 1)) && (seg->endblkno < currdiskreq->ioreqlist->blkno) && (curr->cause > 0)) {
        ...
        if ((currcause == DM_REMAPPED)  // DM_REMAPPED
            && (currblkno >= 0) && (currblkno == (firstblkno - 1))) {
            curr->blkno--;
            ...
            curr->bcount = 1;
            if (((currdisk->read_direct_to_buffer || !(curr->flags & READ)) && ((currdiskreq == seg->recyclereq) || !(disk_buffer_block_available(currdisk, seg, currcause)))) || (currcause < 0)) {
                curr->bcount = 2;
            }
        }
