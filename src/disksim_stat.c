...
void stat_print_set(statgen **statset, int statcnt, char *identstr) {
    ...
    if (avg > 0.0) {
        ...
        //     printf("dev = %f, avg = %f\n", runsquares, avg);
        // }
        delay_fluctuation = runsquares / avg;
        fprintf(outputfile, "%s%s Delay Fluctuation: \t%f\n", identstr, statdesc, delay_fluctuation);
    }

    // **********************************
    // char ss[50];
    // strcpy(ss, identstr);
    // if (!strcmp(strcat(ss, statdesc), "IOdriver Response time")) {
    //     // if (!strcmp(identstr, "IOdriver ") && !strcmp(statdesc, "Response time")) {
