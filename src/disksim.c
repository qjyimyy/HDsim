...
INLINE void addtointq(event *newint) {
    ...
    if (disksim->intq == NULL) {
        ...
        // Cherry: 如果新的事件的时间早于当前 disksim intq 的时间，那么插入到链表前面
...
INLINE static event *getnextevent(void) {
    event *temp;
...
static void prime_simulation() {
    ...
    if (disksim->iotrace) {
        // Cherry: 如果用外部的 trace，读取每一条 trace
...
void disksim_simulate_event(int num) {
    ...
    if ((curr = getnextevent()) == NULL) {
        ...
        } else if ((curr->type >= MEMS_MIN_EVENT) && (curr->type <= MEMS_MAX_EVENT)) {
            io_internal_event((ioreq_event *)curr);
        }
        /* SSD: These are ssd specific events */
        else if ((curr->type >= SSD_MIN_EVENT) && (curr->type <= SSD_MAX_EVENT)) {
            io_internal_event((ioreq_event *)curr);
    ...
    if (strcmp(filename, "0") != 0) {
        ...
        if (strcmp(filename, "stdin") == 0) {
            ...
            if ((disksim->iotracefile = fopen(filename, "rb")) == NULL) {
                fprintf(stderr, "Tracefile %s cannot be opened for some reason.\n", filename);
                exit(1);
            }
        }
    } else {
...
void disksim_setup_disksim(int argc, char **argv) {
    ...
    fprintf(outputfile, "\n**** Date                 : %s", __DATE__);
    fprintf(outputfile, "\n**** DiskSim version      : %s", VERSION4);
    fprintf(outputfile, "\n**** Output file name     : %s", argv[2]);

    if (strcmp(argv[3], "external") == 0) {
        disksim->external_control = 1;
    } else {
        // trace 文件有 ascii, validata, hpl, hpl2, dec, emcsymm, emcbackend, batch 等格式
    ...
    // Cherry: disksim_loadparams 读取配置文件参数
