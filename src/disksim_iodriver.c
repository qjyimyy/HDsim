...
void iodriver_schedule(int iodriverno, ioreq_event *curr) {
    ...
    if (iodrivers[iodriverno]->devices[(curr->devno)].queuectlr != -1) {
        ...
        if ((ctl->maxreqsize) && (curr->bcount > ctl->maxreqsize || curr->blkno % ctl->maxreqsize != 0)) {
            ...
            if (tmp < curr->blkno + curr->bcount) {
                curr->bcount = tmp - curr->blkno;
            } else {
                curr->bcount = curr->bcount > ctl->maxreqsize ? ctl->maxreqsize : curr->bcount;
            }
            if (ctl->oversized) {
                totalreq->next = ctl->oversized->next;
                ctl->oversized->next = totalreq;
...
void iodriver_access_complete(int iodriverno, intr_event *intrp) {
    ...
    if (iodrivers[iodriverno]->devices[(req->devno)].queuectlr != -1) {
        ...
        if ((tmp) && (tmp->next->devno == req->devno) && (tmp->next->opid == req->opid) && (req->blkno >= tmp->next->blkno) && (req->blkno < (tmp->next->blkno + tmp->next->bcount))) {
            ...
            if ((req->blkno + req->bcount) < (tmp->next->blkno + tmp->next->bcount))

            {
                ...
                req->blkno = req->blkno + req->bcount;

                int currOffset = (req->blkno + ctl->maxreqsize) / ctl->maxreqsize * ctl->maxreqsize;

                if (currOffset < tmp->next->blkno + tmp->next->bcount) {
                    req->bcount = currOffset - req->blkno;
                } else {
                    req->bcount = tmp->next->blkno + tmp->next->bcount - req->blkno > ctl->maxreqsize ? ctl->maxreqsize : tmp->next->blkno + tmp->next->bcount - req->blkno;
                }
...
int iodriver_load_logorg(struct lp_block *b) {
    ...
    newlen = disksim->iodriver_info->sysorgs_len ? disksim->iodriver_info->sysorgs_len * 2 : 2;
