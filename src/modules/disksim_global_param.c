...
DISKSIM_GLOBAL_OUTPUT_FILE_FOR_TRACE_OF_IO_REQUESTS_SIMULATED_loader (int
								      ...
								      char *s)
{
  if (NULL != (OUTIOS = fopen (s, "w")))
    {
      strcpy (disksim->outiosfilename, s);
      fprintf (OUTIOS,
	       "simtime,devno,blkno,blkcnt,flags,queueDepth,tagID,curr\n");
    }
...
{
  if (NULL != (disksim->exectrace = fopen (s, "w")))
    {
      disksim->exectrace_fn = strdup (s);
      fprintf (disksim->exectrace,
	       "simtime,tagID,devno,blkno,blkcnt,flags,time\n");
